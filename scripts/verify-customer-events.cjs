...
try {
  ...
  assert.equal(normalized.length, 1, '非法事项、缺少关联客户或空项应被过滤')
  assert.equal(normalized[0].type, '预约已确认', '有效事项应被保留')
  assert.equal(normalized[0].note.length, 3000, '备注应受本地存储长度限制')
  assert.equal(normalized[0].occurredAt, '2026-08-27', '发生日期应保留')

  console.log('客户推进事项测试通过：有效类型、同步结果和本地数据清理均已覆盖。')
} finally {
