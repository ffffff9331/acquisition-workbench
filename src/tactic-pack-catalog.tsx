...
export function TacticPackCatalog({ packs, activePackId, activeIndustryPackId, enabledChannels, selectedChannelId, onActivate }: { packs: GrowthTacticPack[]; activePackId: string; activeIndustryPackId: string; enabledChannels: ChannelId[]; selectedChannelId: ChannelId; onActivate: (packId: string) => void }) {
  ...
  const executionHeading = channelById(previewPack.channelId).kind === 'content' ? '内容与发布' : channelById(previewPack.channelId).kind === 'offline' ? '活动与承接' : '关系与承接'

  const trafficModes = [previewPack.trafficPlan.primaryMode, ...previewPack.trafficPlan.backupModes]
  return <section className="tactic-pack-section">
    <div className="tactic-pack-heading"><div><span><Route size={20} /></span><div><h2>行业打法包</h2><p>把行业规则和渠道能力组织成一条可执行的获客路径。启用后只约束之后的新研究、人工执行和线索承接，不创建固定内容或自动触达。</p></div></div><span className={`package-state ${isActive ? 'installed' : ''}`}>{isActive ? <><PackageCheck size={14} />当前使用</> : '可预览'}</span></div>
    ...
    <div className="tactic-pack-overview"><div><div className="tactic-pack-column-head"><UsersRound size={18} /><h3>适合的客户</h3></div><ul>{previewPack.customerProfile.map((item) => <li key={item}>{item}</li>)}</ul></div><div><div className="tactic-pack-column-head"><Target size={18} /><h3>触发与承接</h3></div><ul>{previewPack.entryScenarios.map((item) => <li key={item}>{item}</li>)}</ul><p className="tactic-call-to-action"><strong>主要动作</strong>{previewPack.callToAction}</p></div><div><div className="tactic-pack-column-head"><Clapperboard size={18} /><h3>{executionHeading}</h3></div><p className="tactic-content-promise"><strong>本次路径要让对方明白</strong>{previewPack.contentPromise}</p><ul>{previewPack.prePublishChecks.map((item) => <li key={item}>{item}</li>)}</ul></div><div><div className="tactic-pack-column-head"><ClipboardCheck size={18} /><h3>线索判断</h3></div><ul>{previewPack.leadFields.map((item) => <li key={item.id}><strong>{item.label}{item.required ? ' *' : ''}</strong>{item.purpose}</li>)}</ul></div><div><div className="tactic-pack-column-head"><ListChecks size={18} /><h3>人工执行与跟进</h3></div><ul>{[...previewPack.channelExecutionSteps, ...previewPack.followUpSteps].map((item) => <li key={`${item.timing}-${item.action}`}><strong>{item.timing}</strong>{item.action}</li>)}</ul></div><div><div className="tactic-pack-column-head"><Gauge size={18} /><h3>流量与验证</h3></div><p className="tactic-experiment-focus"><strong>主要流量方式</strong>{trafficModes.join('；')}</p><p className="tactic-experiment-focus"><strong>开始前门槛</strong>{previewPack.trafficPlan.firstExecutionGate}</p><p className="tactic-experiment-focus"><strong>本轮成功信号</strong>{previewPack.trafficPlan.successSignal}</p><p className="tactic-experiment-focus"><strong>要验证什么</strong>{previewPack.experimentPlan.focus}</p><p className="tactic-experiment-focus"><strong>主要看什么</strong>{previewPack.experimentPlan.primaryMetric}</p><ul>{previewPack.trafficPlan.evidencePriorities.map((item) => <li key={item}>{item}</li>)}</ul><p className="tactic-one-variable"><strong>每轮调整</strong>{previewPack.experimentPlan.oneVariableRule}</p></div></div>
  </section>
}
