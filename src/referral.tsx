...
export function ReferralWorkspace({ data, records, onChange, onBack, onAddLead, onOpenRecord, onToast }: { data: ReferralData; records: ReferralCustomerRecord[]; onChange: (updater: (current: ReferralData) => ReferralData) => void; onBack: () => void; onAddLead: (source?: string) => void; onOpenRecord: (record: ReferralCustomerRecord) => void; onToast: (message: string) => void }) {
  const [section, setSection] = useState<ReferralSection>('overview')
  const [relationView, setRelationView] = useState<ReferralRelationType>('老客户转介绍')
  ...
  const saveRelation = (formData: FormData, relation?: ReferralRelation | null) => {
    ...
    const next: ReferralRelation = {
      ...
      contentPackageId: relation?.contentPackageId || '',
      relationType,
  ...
  return <div className="referral-workspace">
    ...
    {section === 'overview' ? <CommandBand nextRelation={nextRelation} hasRelations={data.relations.length > 0} onNew={() => setEditingRelation(null)} onAdvance={advanceRelation} /> : null}
    {renderSection()}
    {editingRelation !== undefined ? <RelationDialog relation={editingRelation || undefined} defaultType={relationView} settings={data.settings} onClose={() => setEditingRelation(undefined)} onSubmit={saveRelation} /> : null}
...
function RelationDialog({ relation, defaultType, settings, onClose, onSubmit }: { relation?: ReferralRelation; defaultType: ReferralRelationType; settings: ReferralSettings; onClose: () => void; onSubmit: (formData: FormData, relation?: ReferralRelation) => void }) {
