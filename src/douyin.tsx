...
function IdeaDialog({ idea, onClose, onSubmit }: { idea?: DouyinIdea; onClose: () => void; onSubmit: (formData: FormData, idea?: DouyinIdea) => void }) {
  return <div className="dialog-backdrop" role="presentation"><form className="dialog douyin-dialog" onSubmit={(event) => { event.preventDefault(); onSubmit(new FormData(event.currentTarget), idea) }}><div className="dialog-head"><div><h2>{idea ? '编辑选题' : '新建选题'}</h2><p>先记录客户问题和目标动作，采用后再进入内容计划。</p></div><button className="icon-button" type="button" onClick={onClose} aria-label="关闭"><X size={18} /></button></div><div className="form-grid"><label className="field field-wide"><span>选题标题</span><input name="title" required autoFocus defaultValue={idea?.title || ''} placeholder="填写已由真实搜索或客户问题确认的主题" /></label><label className="field field-wide"><span>目标客户问题</span><textarea name="customerProblem" rows={3} defaultValue={idea?.customerProblem || ''} placeholder="客户正在担心、比较或不知道怎么选的事情" /></label><label className="field"><span>内容方向</span><input name="contentDirection" defaultValue={idea?.contentDirection || ''} placeholder="例如：判断方法、案例拆解、方案比较" /></label><label className="field"><span>目标动作</span><input name="targetAction" defaultValue={idea?.targetAction || ''} placeholder="例如：私信咨询、预约到店" /></label><label className="field"><span>选题来源</span><select name="sourceType" defaultValue={idea?.sourceType || '手工'}><option>手工</option><option>AI</option></select></label><label className="field"><span>当前状态</span><select name="status" defaultValue={idea?.status || '待判断'}><option>待判断</option><option>已采用</option><option>暂不采用</option></select></label><label className="field field-wide"><span>标签</span><input name="tags" defaultValue={idea?.tags || ''} placeholder="记录客户阶段、真实场景或搜索来源，用逗号分隔" /></label></div><div className="dialog-foot"><button className="button button-secondary" type="button" onClick={onClose}>取消</button><button className="button button-primary" type="submit">保存<Check size={15} /></button></div></form></div>
}
