...
import { campaignLifecycle, campaignTimingLabel, type CampaignLifecycle } from './traffic-campaign'
...
export function trafficCampaignQueue(topics: GeneratedTopicCandidate[], performanceByOpportunity: Record<string, OpportunityChannelPerformance[]>, now: string): TrafficCampaignQueue {
  const items = topics.flatMap((opportunity): TrafficCampaignQueueItem[] => {
    ...
    }), { inquiries: 0, leads: 0, qualifiedLeads: 0, customers: 0 })
    ...
    return [{ opportunity, lifecycle, action: days !== null && days <= 1 ? '回收渠道结果' : '查看执行进度', timing, total, priority: days !== null && days <= 1 ? 0 : 3 }]
