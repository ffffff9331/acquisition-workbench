...
export function OfflineWorkspace({ data, records, onChange, onBack, onAddLead, onOpenRecord, onToast }: { data: OfflineData; records: OfflineCustomerRecord[]; onChange: (updater: (current: OfflineData) => OfflineData) => void; onBack: () => void; onAddLead: (source?: string) => void; onOpenRecord: (record: OfflineCustomerRecord) => void; onToast: (message: string) => void }) {
  ...
  return <div className="offline-workspace">
    ...
    {renderSection()}
    {editingTask !== undefined ? <TaskDialog task={editingTask || undefined} defaultType={activityView} settings={data.settings} onClose={() => setEditingTask(undefined)} onSubmit={saveTask} /> : null}
